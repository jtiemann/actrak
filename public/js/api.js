...
class ApiClient {
      ...
      try {
        ...
        if (retryCount < this.maxRetries && 
            (error.name === 'TypeError' || // Network error
             (error.status && error.status >= 500))) { // 5xx status
  ...
  /**
   * Get all activity types for current user
  ...
  async getActivityTypes() {
    ...
    return this.request(`/activity_types/${user.id}`);
  }
  ...
  async getActivity(activityId) {
    return this.request(`/activity_types/details/${activityId}`);
  }
  ...
  async createActivity(name, unit, category = 'other') {
    ...
    const result = await this.request('/activity_types', 'POST', {
      userId: user.id,
      name,
      unit,
      isPublic: false
    ...
    // Clear activity_types cache
    this.clearCache(`/activity_types/${user.id}`);
  ...
  async updateActivity(activityId, name, unit, isPublic = false) {
    ...
    const result = await this.request(`/activity_types/${activityId}`, 'PUT', {
      name,
      unit,
      isPublic
    });
  ...
  async deleteActivity(activityId) {
    ...
    const result = await this.request(`/activity_types/${activityId}`, 'DELETE');
  ...
  async getActivityCategories() {
    ...
    return this.request(`/activity_types/categories/${user.id}`);
  }
